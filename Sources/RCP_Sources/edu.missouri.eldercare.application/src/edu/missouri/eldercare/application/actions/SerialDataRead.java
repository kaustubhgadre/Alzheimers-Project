...
import edu.missouri.eldercare.application.utilities.CSVWriter;
...
public class SerialDataRead implements Runnable {
	...
	/*
	 * private CommPortIdentifier portId; private SerialPort serialPort;
	 */
	 ...
	 * this.portId = portId; this.elderCareView = elderCareView; this.file =
	 * file; initParams(); new Thread(this).start(); }
	 *//**
	 * @param portId
	 ...
	 * TooManyListenersException, UnsupportedCommOperationException { serialPort
	 * = (SerialPort) portId.open("Elder Care", 2000); inputStream =
	 * serialPort.getInputStream(); serialPort.addEventListener(this);
	 ...
	 * SerialPortEvent.DSR: case SerialPortEvent.RI: break;
	 * 
	 * case SerialPortEvent.OUTPUT_BUFFER_EMPTY: // No data recieved break; case
	 * SerialPortEvent.DATA_AVAILABLE: byte[] readBuffer = new byte[8]; int
	 * numBytes = 0; try { while (inputStream.available() > 0) { numBytes =
	 * inputStream.read(readBuffer); String frameData = new String(readBuffer);
	 * // System.out.println(frameData); processFrameData(frameData, numBytes);
	...
	/*
	 * private void showData(final int[] finalData) throws
	 * RowsExceededException, WriteException, IOException, BiffException {
	 * Display.getDefault().asyncExec(new Runnable() {
	 * 
	 * public void run() { if (elderCareView instanceof ElderCareView) {
	 * org.eclipse.swt.widgets.Label[] labels = ((ElderCareView) elderCareView)
	 * .getChildren(); for (int i = 0; i < finalData.length; i++) { String value
	 * = Integer.toString(finalData[i]); if (finalData[i] == 1) { labels[i]
	 * .setBackground(new Color(null, 237, 28, 36)); Composite parent =
	 * labels[i].getParent(); if (parent instanceof Canvas) { Canvas canvas =
	 * (Canvas) parent; canvas.setBackground(new Color(null, 237, 28, 36)); int
	 * count = Integer.parseInt(labels[i] .getText()) + 1; labels[i].setText(""
	 * + count); // writeIntoExcel(workbook, labels[i], i, // value);
	 * 
	 * } } else { Color backGroundColor = (Color) labels[i]
	 * .getData("greenBack"); labels[i].setBackground(backGroundColor);
	 * Composite parent = labels[i].getParent(); if (parent instanceof Canvas) {
	 * Canvas canvas = (Canvas) parent; canvas.setBackground(backGroundColor);
